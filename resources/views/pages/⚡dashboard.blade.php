...
<div class="flex w-full flex-col gap-6">
    ...
    <div class="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        ...
        @if($this->business['visibility']['canViewCosts'])<flux:card><flux:text>Costo de ramos vendidos</flux:text><flux:heading size="xl" class="mt-2">{{ $this->business['cost_base'] === null ? 'Pendiente de regularizar' : $currency->symbol.' '.Number::format((float) $this->business['cost_base'], precision: $currency->decimal_places, locale: 'es') }}</flux:heading></flux:card>@endif
        @if($this->business['visibility']['canViewProfit'])<flux:card><flux:text>Margen bruto</flux:text><flux:heading size="xl" class="mt-2">{{ $this->business['gross_margin_base'] === null ? 'Pendiente de regularizar' : $currency->symbol.' '.Number::format((float) $this->business['gross_margin_base'], precision: $currency->decimal_places, locale: 'es') }}</flux:heading></flux:card>@endif
